...
public class NetreikOreBlock extends FantasyFusionsModElements.ModElement {
	@ObjectHolder("fantasy_fusions:netreik_ore")
	public static final Block block = null;
	public NetreikOreBlock(FantasyFusionsModElements instance) {
		super(instance, 8);
	}
	...
	public static class CustomBlock extends Block {
		...
		public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
			...
			return Collections.singletonList(new ItemStack(NetreikLeafItem.block, (int) (1)));
		}
	}
	@Override
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("netreik_ore", "netreik_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.MAGMA_BLOCK.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 4)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(60, 0, 0, 50))));
		}
	}
}
